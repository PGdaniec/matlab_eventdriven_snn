function [rate,sidx,stimes,debugValue] = spikingnet_del(inp)
    % features:
    % - inputs
    % - refactory time
    % - weight adaption
    ...
    synW_mat = [[ones(ke,ne)*synW_val.*synW_exc_f; zeros(max(ke,ki)-ke,ne)]...
                [ones(ki,ni)*synW_val.*synW_inh_f; zeros(max(ke,ki)-ki,ni)] ]; 
    ...
    synW_inc_exc_scale = synW_inc_scale * abs(synW_val * mean(synW_exc_f(:)));
    synW_inc_inh_scale = synW_inc_scale * abs(synW_val * mean(synW_inh_f(:)));
    synW_dec_exc_scale = synW_dec_scale * abs(synW_val * mean(synW_exc_f(:)));
    synW_dec_inh_scale = synW_dec_scale * abs(synW_val * mean(synW_inh_f(:)));
    synWin_inc_scale   = synW_inc_scale * -j0/sqrt(kin+max(ki,ke)) * mean(synW_ext_f(:));
    synWin_dec_scale   = synWin_inc_scale;
    ...
    del_writeptr  = 1;
    del_readptr   = 1;
    del_capacity  = max(inp.ki,inp.ke)*n;
    del_nFilled   = 0;
    del_nFilledMax= 0;
    delay_phi = 0.0002 / T_free;
    del_FROMJ       = 1;
    del_AT_NEXT_PHI = 2;
    del_delayedSpike_fromJ_atNextPhi = zeros(2,del_capacity);
    % main loop ###########################################################
    % #####################################################################
    s = 1;
    while s ~= nspike+1
        ...
        ext_input_present = ...
           ...
           phishift*T_free             >  ext_input_time(ext_input_k);  % if next external input time smaller than sim time
        % external input
        if( (ext_input_present && del_nFilled ==  0)       || ...  % external input present and no delayed spikes in queue
            (ext_input_present                                     && ...       % or external input present and delayed spikes in queue, but they come later
             del_nFilled                 >  0                      && ...                                      % if there are delayed spikes
             ext_input_time(ext_input_k) <  del_delayedSpike_fromJ_atNextPhi(del_AT_NEXT_PHI,del_readptr)) ... % and they come after the external spike
          )
        ...
        % make internal calculation    
        else  
            % If next delayed spike in queue is at time, read and process it
            if(phishift      > del_delayedSpike_fromJ_atNextPhi(del_AT_NEXT_PHI,del_readptr) && ...
               del_nFilled   > 0) % if there is something to read
                % set phi
                phishift = del_delayedSpike_fromJ_atNextPhi(del_AT_NEXT_PHI,del_readptr);
                j        = del_delayedSpike_fromJ_atNextPhi(del_FROMJ,del_readptr);
                % use normal internal spiking
                postidx = postidx_precomp(j,:);
                % evaluate phase transition curve
                ptc_fun(postidx,[] , false);
                increment_read_delayedSpikePtr();
            else
                write_delayedSpike();
    ...
    debugValue.del_nFilledMax             = del_nFilledMax;
        ...
        for i_l = postidx
            ...
            if(use_refactoryTime && phishift-lastSpikeTimeOfNeuron(i_l) < refactory_phi)
                ...
                if(~isExtInput)
                    ...
                    % depends on j index, as here we habe output synapses of j
            ...
            if(n_idx ~= 0 && phishift-lastSpikeRX(atTime,j) < deltaPhi_synW_change)
                ...
                if(n_idx>ne)
                    ...
                    synW_mat(n_syn_idx,n_idx) = max(  synW_mat(n_syn_idx,n_idx) ...
                                                    - synW_inc_inh_scale,-synW_inc_limit_inh);    
                else
                    % for exc. shall be positive or 0
                    synW_mat(n_syn_idx,n_idx) = min(  synW_mat(n_syn_idx,n_idx) ...
                                                    + synW_inc_exc_scale,+synW_inc_limit_exc);    
                end         
            ...
            if(input_syn_idx ~= 0 && phishift-lastInputSpikeRX(atTime,j) < deltaPhi_synW_change)
                ...
                if(sign(synWin_mat(input_syn_idx,j)) == -1)
                    ...
                    synWin_mat(input_syn_idx,j) = max(  synWin_mat(input_syn_idx,j) ...
                                                      - synWin_inc_scale,-synW_inc_limit_inh);    
                else
                    % for exc. shall be positive or 0
                    synWin_mat(input_syn_idx,j) = min(  synWin_mat(input_syn_idx,j) ...
                                                      + synWin_inc_scale,+synW_inc_limit_exc);    
                end       
            end
        end
	...
	% #####################
    function write_delayedSpike()
        if(del_nFilled < del_capacity)
            del_nFilled = del_nFilled + 1;
            del_delayedSpike_fromJ_atNextPhi(:,del_writeptr) = [j; phishift+delay_phi];
            del_writeptr = mod(del_writeptr,del_capacity) + 1;
            del_nFilledMax = max(del_nFilledMax,del_nFilled);
        else
            error(['Write Buffer too small at step s = ' num2str(s)]);
        end
        
    end
    % #####################
    function increment_read_delayedSpikePtr()
        % there must be at least one written data
        del_readptr = mod(del_readptr,del_capacity) + 1;
        del_nFilled = del_nFilled - 1;
    end
end
